...
public final class BloatDown
{
	...
	/**
	 * Walks from {@code start} in the given direction, one tile at a time, until collision blocks
	 * further movement - i.e. simulates where Bloat will come to rest (and stomp) if it keeps
	 * walking its current heading. Uses {@link WorldArea#canTravelInDirection} against the live
	 * collision map, same as the game's own pathing check.
	 */
	private static WorldPoint getFinalDestination(Client client, WorldPoint start, Supplier<int[]> offsetsSupplier)
	{
		...
		WorldPoint wl = start;
		WorldArea wa = new WorldArea(start, 1, 1);
		while (wa.canTravelInDirection(wv, offsets[0], offsets[1]))
		{
			wl = wl.dx(offsets[0]).dy(offsets[1]);
			wa = new WorldArea(wl, 1, 1);
		}
		return wl;
	}
